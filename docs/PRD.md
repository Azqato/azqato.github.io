...
| v3.36.0 — **"MAG 10" filter** (renamed from the "FANG+" placeholder once the owner supplied the list). Fixed 10-ticker watchlist: AAPL, AMD, AMZN, AVGO, GOOGL, META, MSFT, NFLX, NVDA, TSLA. A client-side toggle sourced specifically from the S&P 500 data (owner instruction) — switches the active universe to S&P 500 if needed, then filters to these 10 rows, so each stock's score/tier reflects its percentile among the full 500-stock universe. ANDs with the existing tier filter and search box. See ROADMAP.md for the full plan | Next | Planned |
| v3.37.0 — ETFs universe rating methodology review. **Unscoped, awaiting owner input** — do not start design or implementation until the owner specifies what to address. Current methodology to be presented in full first | After v3.36 | Planned |
