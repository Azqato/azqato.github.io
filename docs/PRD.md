...
| v3.31.0 — Screener: five-tier grading scale (owner-decided 2026-07-03). Replace Pass/Watch/Fail with a tier list on the existing 0-100 score: **S** (dark green, 80+), **A** (light green, 65-79), **B** (yellow, 50-64), **C** (light red, 35-49), **F** (dark red, <35); the table column and filter chips are labeled **Tier** (replacing Verdict). Restores resolution the 3-tier scale hides (a 79 and a 51 currently read identically as Watch), and tier-list vocabulary is native to the site's Twitch/Discord audience while carrying no buy/sell advice connotation (an F means "bottom of this list", consistent with the percentile-forced curve). Decision history: the owner's first proposal (Strong Buy through Strong Sell) was dropped for conflicting with this PRD's non-goals ("not a buy/sell signal generator", "no financial advice language"). Needs 2 new color tokens (dark green, light red variants) in DESIGN.md, updated filter chips, popup, methodology copy, and FAQ answers | After v3.30 | Planned |
