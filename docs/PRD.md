...
| Music          | `music.html`          | DJ stage visualizer, Mixcloud mixes, and native track player |
...
- **F12: DJ Stage Visualizer (`music.html`).** Full-viewport canvas rendering a 3D-styled DJ stage (Brooklyn Mirage-inspired 3-screen layout: one 42%-wide center screen plus two 22%-wide side panels, DJ booth, fire jets, lasers, crowd, truss lighting). Ten selectable modes (`Bars`, `Volumetric`, `Stars`, `Vortex`, `Squares`, `Origami`, `Tunnel`, `Ghost`, `Fence`, `Noise`); five are exposed as visible buttons (Stars, Vortex, Squares, Tunnel, Fence) and cycle automatically at random (no immediate repeat) every ~30s, the rest remain in code but are hidden from the UI and excluded from auto-cycling. Screens, fire, and lasers all react to whatever audio source is currently feeding the shared frequency analyser (native player audio if playing, otherwise a synthetic idle wave). Cinematic rendering pass: DPR-aware canvas, metal screen bezels, glass gloss and LED scanline overlays, cheap downsample-buffer bloom, floor reflections, prerendered vignette/letterbox grade, layered additive fire and laser rendering with distance falloff.
- **F13: Native Track Player with Kick Detection (`music.html`).** In-page `<audio>` element routed through the Web Audio API (`createMediaElementSource` → `AnalyserNode` → destination) so a locally loaded track plays back and simultaneously drives the visualizer, no external embed required. UI: a player card in the stage console with play/pause, track title, elapsed/remaining time, and a draggable seek bar. Feeds two independent analysers: a general-purpose one (smoothed, coarse resolution) for the broad visualizer reactivity used throughout the page, and a dedicated high-resolution, unsmoothed, ~30-150Hz-bandpassed analyser used only for kick detection. The kick detector uses onset (rate-of-rise) detection rather than absolute-level detection, since a heavily-mastered/loud mix keeps the bass band near ceiling almost constantly, making level-based detection unreliable; parameters were tuned by extracting the bass band from a real test track with `ffmpeg` and sweeping detector settings against the track's independently-measured tempo (autocorrelation of the onset curve) until detected hits matched the real beat spacing. Detected kicks drive a beat-synced zoom/brightness/bloom pulse on all three stage screens and scale the laser fixtures' active beam count (1-2 beams when quiet, up to 6-8 when loud). A separate, rarer "drop" flash fires only when the current loudness is a statistical outlier (top ~20%) above a slowly-adapting running baseline, gated to a kick hit and a 5s minimum cooldown, so it reads as an occasional peak-moment accent rather than a per-beat strobe.
...
| Native track player has no deployed audio | `music.html`'s `<audio>` element points at a local test file (`music/2025 Mix Final.mp4`) that is gitignored and never pushed, so the play button is untested and non-functional on the live site | Point `src` at a real, licensed, hosted track (or a small set of tracks with a picker) before or as part of any release that surfaces this player publicly |
