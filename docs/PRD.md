...
- Scoring model v2: 8 metrics in four weighted pillars (Growth 40, Valuation 20, Profitability 20, Balance sheet 20), ranked against peers with a top/bottom-28% points clamp, missing data scoring a hard zero, total /100, mapped to S+/S/A/B/C/F tiers (S+ = a perfect 100 score; S = top 10%, A = next 10%, B = 20–50%, C = 50–75%, F = bottom 25% by rank within the loaded list; boundary ties round up)
...
| `grossMargin` | number | Gross margin TTM (%) |
| `netMargin` | number | Net (profit) margin TTM (%) |
...
### Screener Scoring Model (v2, shipped v3.30.0)

A relative, percentile-based model. Each stock is ranked against its loaded peers on eight metrics in four weighted pillars totaling 100 points. (v2 replaced the five-metric v3.15 model, which had double-counted valuation, ignored the TTM and margin metrics the site's doctrine teaches, and let a quarter of the list share perfect metric scores; that in turn had replaced the v3.12 absolute-threshold model.)

| Pillar | Weight | Metric | Direction | Value ranked |
|--------|--------|--------|-----------|--------------|
| Growth | 10 | Revenue Growth TTM | higher is better | `revTTM` |
| Growth | 10 | Revenue Growth FWD | higher is better | `revFwd` |
| Growth | 10 | EPS Growth TTM | higher is better | `epsTTM` |
| Growth | 10 | EPS Growth FWD | higher is better | `epsFwd` |
...
| Profitability | 10 | Gross Margin | higher is better | `grossMargin` |
| Profitability | 10 | Net Margin | higher is better | `netMargin` |
...
**Percentile → points:** `points = clamp(20 × (percentile − 0.28) / 0.44, 0, 20)`, then scaled by the metric's weight. Bottom 28% scores 0; top 28% scores full marks; the median scores half. Ties take the average rank. The 28% clamp was calibrated on the final live 8-metric feeds (2026-07-03) to the owner's target of ~1 perfect score in the Nasdaq 100 and ~5 in the S&P 500, ties allowed (live at ship: 2 tied at 100 in the Nasdaq 100, exactly 5 in the S&P 500, vs 4 and 11 under the old quartile clamp). An interim 15% clamp, fitted before the margin fields landed in the feeds, proved too tight once all eight metrics were live (top scores stalled around 99 with no perfect scores).
...
**Factors chip:** count of the eight scored metrics earning 15+ of 20 percentile points (the upper part of the pack), out of a fixed /8; a missing metric counts as a miss.
...
**Per-stock popup:** clicking any row opens a focused breakdown for that stock — all eight scored metrics with value, percentile, and weighted points (e.g. "7.2/10"), color-coded, with the total score and tier. Missing metrics show 0 points in dark red. Reuses the modal component.
...
Revenue Growth TTM, Revenue Growth FWD, EPS Growth TTM, EPS Growth FWD, P/E FWD, PEG FWD, Total Cash, Total Debt, RSI, 52-Week Range, Gross Margin, and Net Margin. The ten growth, valuation, profitability, and balance sheet metrics drive individual stock decisions (the screener scores eight of them across four weighted pillars). RSI and the 52-Week Range are technical signals used to time index and ETF purchases, tracked on stock watchlists for context only.
