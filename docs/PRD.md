...
| v4.5.0 — Site-wide mobile-friendliness pass: device-width audit (375/414/768px) across all 9 pages, screener-specific decisions (sticky Ticker/Tier/Score columns, universe-switcher behavior at narrow widths), touch-target sizing; sequenced after v3.35.0 so the table CSS fix lands first (see ROADMAP.md) | TBD | Planned |
...
| v3.34.10 — Screener bug (in progress). Two further reports after v3.34.8 shipped confirm the scroll issue isn't fully fixed: the same friend still can't scroll on Opera at 1280×1024 (a 10-row ETFs table, ruling out row-count/height), and the owner reports the table "stops scaling" when narrowing a Chrome window. Diagnosis (read-only so far, live-site width sweep 1030-1400px in headless Chrome): the v3.34.8 box-sizing fix is confirmed correct at every width tested, but headless Chromium cannot reproduce what real Opera/Chrome show — pointing at Chrome/Opera's default **overlay scrollbar** (hover-only, invisible in a static look) as the real remaining cause, not a sizing bug. Plan drafted (persistently-visible scrollbar styling, wheel-to-horizontal-scroll redirect, right-edge fade affordance) but **not yet executed**, per owner instruction to present the plan first. See ROADMAP.md for the full record | 2026-07-04 | In progress |

### Feature Breakdown by Phase

**v3.x (current):** Screener with daily data, relative percentile scoring, methodology popup, documentation consolidation.

**Queue head (updated 2026-07-04):** v3.28.0 GVD universes, v3.29.0 rank-based tier scale, v3.30.0 scoring model v2, v3.31.0 margins-out re-weighting, v3.32.0 pipeline cleanup, v3.33.0 ETFs universe, v3.34.0 International universe, v3.34.5 workflow timing review, v3.34.6 International same-company duplicate fix, v3.34.7 International name-primary display, and v3.34.8 horizontal-scroll bug fix are shipped — all seven screener universes are live, the daily pipeline is re-scheduled (Nasdaq 100 21:30 → ETFs 22:00 → S&P 500 22:30 → GVD 23:00 → International 23:30 UTC, all Mon-Fri, 30-min gaps throughout, anchored to the latest-possible market close), the International list's top 100 correctly represents 100 distinct companies, the International universe leads with company names, and the screener table's box-sizing scrolls correctly at every resolution — **but two further reports show the scrollbar itself is still not discoverable/usable for real users (likely an invisible overlay-scrollbar issue, not a sizing bug); v3.34.10 plan drafted, awaiting go-ahead to execute.** Next: v3.34.10 scrollbar-visibility fix (plan ready, pending owner approval), then v3.35.0 methodology audit and table-display fixes (owner-flagged, root cause already identified), then v3.36.0 "FANG+" filter (owner-requested, blocked on the owner's ticker list), then v3.37.0 ETFs rating methodology review (owner-flagged 2026-07-04, unscoped, awaiting specifics — likely to jump priority once scoped), then v4.0.0 sparklines, v4.1.0-v4.4.0 content releases, then v4.5.0 mobile-friendliness pass (owner-requested 2026-07-04, appended to the end of the queue). Constituent sourcing decision (2026-07-03): index universes stay on Wikipedia (updates within days of index changes); ETF universes use Vanguard's holdings API (authoritative but month-lagged, acceptable because the fund's published holdings are the universe definition). Switching the S&P 500 to VOO holdings was considered and rejected for the freshness reason. Note the dependency: the v4.0.0 sparklines mine the git history of the committed data feeds, so keeping generated data files in the repo is an intentional design choice, not tech debt.
