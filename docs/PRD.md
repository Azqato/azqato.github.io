...
| v4.0.0 — **Screener responsive redesign & site-wide mobile-friendliness pass, reprioritized to the front of the queue.** Owner decision: auto-hide column groups at narrower widths (extending the existing Columns menu with width-based defaults) rather than a card-view rebuild or fluid/shrink-to-fit sizing — Ticker/Tier/Score/Factors always visible, Growth/Performance/Valuation/Balance-Sheet/Technicals groups progressively hide as the window narrows, eliminating the need to scroll at all. Scope audit confirmed only the screener has this problem (all 8 content pages already reflow correctly with no fixed-width elements); the toolbar row already wraps correctly via `flex-wrap` (confirmed in headless Chrome). See ROADMAP.md for the full plan | Next | Planned |
| v3.35.0 — Screener Methodology audit and table/wrap display fixes (see ROADMAP.md for the full plan: `.table-wrap` CSS bug fix, scroll-vs-wrap decision per table, content-currency audit of `#methodStock`/`#methodEtf` against the shipped scoring model, mobile visual polish pass) | After v4.0.0 | Planned |
...
| v4.1.0 — Screener score history sparklines (mine screener.json git history for per-stock score trends) | After v3.37 | Planned |
