...
| v3.37.0 — ETFs universe rating methodology: **scoring change shipped 2026-07-04, `indices.html` doctrine write-up still open.** Current methodology presented and cross-checked against `indices.html`'s own doctrine, surfacing 3 gaps (VIX/AAII structurally can't be per-fund metrics; Price vs 200-Day MA is scored but undocumented on `indices.html`, owner decided to document it rather than remove it, not yet written; YTD is doctrine-named but unscored while 1Y isn't doctrine-named but is scored — remains unaddressed, owner declined the recommendation to promote YTD). Owner removed Yield and Expense Ratio from scoring (demoted to weight-0 context, still displayed) and weighted up the two longest return horizons instead: 5-Year 10→20, 10-Year 10→20, keeping 1-Year at 10. Final model: Technicals 50 (unchanged) + Performance 50 (1Y 10, 5Y 20, 10Y 20) = 100. `ETF_METRICS`, `ETF_POPUP_METRICS`, column header titles, and the `#methodEtf` popup all updated. Verified via headless Chrome (ETFs universe: 10/10 scored, tiers 1S/1A/3B/3C/2F, Factors chip now `/6`) and a Nasdaq 100 regression check (exact match to the v3.31.0 baseline). See ROADMAP.md for the full record | 2026-07-04 | Complete (scoring); indices.html write-up still Planned |
| v4.0.0 — **Screener responsive redesign, methodology table fix & site-wide mobile-friendliness pass** (absorbs the retired v3.35.0). Owner decision: auto-hide column groups at narrower widths (extending the existing Columns menu with width-based defaults) rather than a card-view rebuild or fluid/shrink-to-fit sizing — Ticker/Tier/Score/Factors always visible, Growth/Performance/Valuation/Balance-Sheet/Technicals groups progressively hide as the window narrows, eliminating the need to scroll at all. Also fixes the methodology modal's `.table-wrap` CSS bug and audits `#methodStock`/`#methodEtf` content for drift (merged from v3.35.0). Scope audit confirmed only the screener has the scroll problem (all 8 content pages already reflow correctly). See ROADMAP.md for the full plan | After v3.37 | Planned |
...
**Queue head (updated 2026-07-04):** v3.28.0 GVD universes, v3.29.0 rank-based tier scale, v3.30.0 scoring model v2, v3.31.0 margins-out re-weighting, v3.32.0 pipeline cleanup, v3.33.0 ETFs universe, v3.34.0 International universe, v3.34.5 workflow timing review, v3.34.6 International same-company duplicate fix, v3.34.7 International name-primary display, v3.34.8 horizontal-scroll bug fix, and v3.36.0 "MAG 10" filter are shipped — all seven screener universes are live, the daily pipeline is re-scheduled (Nasdaq 100 21:30 → ETFs 22:00 → S&P 500 22:30 → GVD 23:00 → International 23:30 UTC, all Mon-Fri, 30-min gaps throughout, anchored to the latest-possible market close), the International list's top 100 correctly represents 100 distinct companies, the International universe leads with company names, the screener table's box-sizing scrolls correctly at every resolution, and a fixed 10-stock "MAG 10" watchlist toggle scores against the full S&P 500. v3.34.10's scrollbar-visibility fix is superseded; v3.35.0 is retired and merged into v4.0.0; v3.37.0's ETF scoring change (Yield/Expense Ratio removed, 5Y/10Y weighted up) has shipped, with its `indices.html` doctrine write-up (Price vs 200-Day MA) still open. **Next: v4.0.0** (screener responsive redesign + methodology table fix + mobile pass, merged scope), then v4.1.0 sparklines, v4.2.0-v4.5.0 content releases. Constituent sourcing decision (2026-07-03): index universes stay on Wikipedia (updates within days of index changes); ETF universes use Vanguard's holdings API (authoritative but month-lagged, acceptable because the fund's published holdings are the universe definition). Switching the S&P 500 to VOO holdings was considered and rejected for the freshness reason. Note the dependency: the v4.1.0 sparklines mine the git history of the committed data feeds, so keeping generated data files in the repo is an intentional design choice, not tech debt.
