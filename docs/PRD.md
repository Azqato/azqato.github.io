...
| v3.31.0 — Screener: five-tier verdict scale (owner-proposed 2026-07-03). Replace Pass/Watch/Fail with five bands on the existing 0-100 score, colored dark green → light green → yellow → light red → dark red (proposed cuts: 80+ / 65-79 / 50-64 / 35-49 / <35). Restores resolution the 3-tier scale hides (a 79 and a 51 currently read identically as Watch). OPEN DECISION before build: the owner's proposed labels (Strong Buy / Buy / Hold / Sell / Strong Sell) conflict with this PRD's non-goals ("not a buy/sell signal generator", "no financial advice language") and read as broker recommendations on a percentile-relative forced curve, where the bottom tier of a hand-picked list (e.g. the Growth 100) is not a meaningful "Strong Sell". Alternative preserving the 5 tiers and color ramp with screen-relative labels (e.g. Top Tier / Strong / Middle / Weak / Bottom Tier) to be decided with the owner at build time. Needs 2 new color tokens (dark green, light red variants) in DESIGN.md, updated filter chips, popup, methodology copy, and FAQ answers | After v3.30 | Planned (label decision open) |
