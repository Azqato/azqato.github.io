...
| v3.37.0 — ETFs universe rating methodology review, **in progress.** Current methodology presented and cross-checked against `indices.html`'s own doctrine, surfacing 3 gaps (VIX/AAII structurally can't be per-fund metrics; Price vs 200-Day MA is scored but undocumented on `indices.html`; YTD is doctrine-named but unscored while 1Y isn't doctrine-named but is scored). Owner decided to remove Yield and Expense Ratio and document Price vs 200-Day MA on `indices.html` rather than remove it from the screener. Recommendation given for the freed 20 points (promote YTD to a scored metric). Awaiting owner's reallocation decision before touching `screener.js`; the `indices.html` doc addition can proceed independently. See ROADMAP.md for the full record | After v3.36 | Planned |
