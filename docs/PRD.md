...
| v3.35.0 — Screener Methodology audit and table/wrap display fixes (see ROADMAP.md for the full plan: `.table-wrap` CSS bug fix, scroll-vs-wrap decision per table, content-currency audit of `#methodStock`/`#methodEtf` against the shipped scoring model, mobile visual polish pass) | After v3.34.6 | Planned |
| v4.0.0 — Screener score history sparklines (mine screener.json git history for per-stock score trends) | After v3.35 | Planned |
